...
int main() {
    ...
    const int days_of_week = 7;  // Days of the week

    // Print header
    attron(COLOR_PAIR(1));
    mvprintw(0, 0, "%s %s %s %s %s %s %s", DAY_NAMES[0], DAY_NAMES[1], DAY_NAMES[2], DAY_NAMES[3], DAY_NAMES[4], DAY_NAMES[5], DAY_NAMES[6]);
    ...
    for (int minutes_interval = 0; minutes_interval < minutes_intervals; ++minutes_interval) {
        for (int day_of_week = 0; day_of_week < days_of_week; ++day_of_week) {
            ...
            TimeAndDate currentTime = TimeAndDate::build(time_of_day, day_of_week + 1, 2024);
            ...
            mvprintw(y + minutes_interval, x, "%s", is_busy ? "true" : "false");
